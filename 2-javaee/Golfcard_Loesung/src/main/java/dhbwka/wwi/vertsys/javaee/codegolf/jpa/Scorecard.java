...
public class Scorecard implements Serializable {
    ...
    public int getStableford() {
        ...
        for (Stroke stroke : this.getStrokes()) {
            if (stroke.getAmount() < 1) {
                continue;
            }
            
            if (stroke.getAmount() <= par - 3) {
                stableford += 5;
            } else if (stroke.getAmount() == this.par - 2) {
                stableford += 4;
            } else if (stroke.getAmount() == this.par - 1) {
                stableford += 3;
            } else if (stroke.getAmount() == this.par) {
                stableford += 2;
            } else if (stroke.getAmount() == this.par + 1) {
                stableford += 1;
            }
